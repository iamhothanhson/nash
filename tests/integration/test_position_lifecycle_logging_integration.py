...
class TestFullLifecyclePositionLoggingIntegration:
    ...
    def _read_positions_history_journal(self, month: str) -> list[dict]:
        """month: YYYY-MM -> position_history/MM-YYYY-positions-history.json"""
        segs = month.split("-")
        assert len(segs) == 2
    ...
    def test_full_lifecycle_daily_log_line_by_line(self) -> None:
        ...
        emit_mode_event(
            ...
            "Size: 330.13 USDT | Margin: 47.16 | SL: 248.66 (-0.58%) | TP1: 252.60 (1.00%) | "
            "TP2: 255.10 (2.00%) | TP3: 257.60 (3.00%) | Risk: 1.90 USDT",
        )
        emit_mode_event("demo", "TAOUSDT", "LONG", "TP1 HIT", "Price: 252.60 | Closed: 165.07 USDT | Remaining: 165.07 USDT | PNL: +1.65 USDT")
        emit_mode_event("demo", "TAOUSDT", "LONG", "BREAKEVEN", "SL → Entry (250.10) | Remaining: 165.07 USDT | Risk: 0 USDT")
        emit_mode_event("demo", "TAOUSDT", "LONG", "TP2 HIT", "Price: 255.10 | Closed: 99.04 USDT | Remaining: 66.03 USDT | PNL: +1.98 USDT")
        emit_mode_event("demo", "TAOUSDT", "LONG", "TP3 HIT", "Price: 257.60 | Closed: 66.03 USDT | Remaining: 0.00 USDT | PNL: +1.98 USDT")
        emit_mode_event(
            "demo",
            "TAOUSDT",
            "LONG",
            "CLOSE",
            "Size: 330.13 USDT | Margin: 47.16 USDT | Entry: 250.10 | Exit: 257.60 | Duration: 20.7 min | Total PNL: +5.61 USDT",
        )
        ...
        after_open = self._read_positions_history_journal("2026-04")
        ...
        assert len(got) == 9
        assert "[DEMO] [OPEN] TAOUSDT [LONG]" in got[3]
        assert "[DEMO] [CLOSE] TAOUSDT [LONG]" in got[8]
        opened = self._read_positions_history_journal("2026-04")
        assert len(opened) == 1
        assert opened[0]["status"] == "Closed"
        assert opened[0]["opened"] == "Apr-27-2026 09:00:02"
        assert opened[0]["closed"] == "Apr-27-2026 09:20:44"
        assert float(opened[0]["pnl_usdt"]) == 5.61
        assert float(opened[0]["balance_usdt"]) == 338.61
        assert not (self.tmp_path / "position_history" / "closed-positions-2026-04.json").exists()
